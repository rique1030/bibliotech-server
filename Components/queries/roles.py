...
class RoleQueries:
    ...
    --------------------------------------------------
    """
    async def insert_multiple_roles(self, roles: list):
        """
        Inserts multiple roles into the database.
        """
        async with self.session_factory() as session:
            try:
                await session.bulk_insert_mappings(Role, roles)
    ...
    async def get_all_roles(self):
        ...
        async with self.session_factory() as session:
            try:
                result = await session.query(Role).all()
                if result is None:
                    return []
                return await self.query_helper.model_to_dict(result)
            except Exception as e:
    ...
    async def update_roles(self, roles: list):
        ...
        async with self.session_factory() as session:
            try:
                await session.bulk_update_mappings(Role, roles)
                await session.commit()

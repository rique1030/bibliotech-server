...
class BaseQuery:
    def __init__(self, session_factory: async_sessionmaker[AsyncSession]) -> None:
        ...
        self.book_cover_path = os.path.join(self.storage_path, "images", "book_covers")
        self.user_photos_path = os.path.join(self.storage_path, "images", "user_photos")
    ...
    async def execute_query(self, operation, *args, **kwargs):
        async with self.session_factory() as session:
            try:
                ...
                if not "data" in result or not "message" in result:
                    print(result)
    ...
    async def get_pymysql_message(self, e):
        ...
        if "for key" in message:
            ...
            VALUE = message.split('Duplicate entry')[1].split('for key')[0].strip()
